...
Group=[-1001231444581,-1001517843177,-1001183315065,-1001293483771,-1001362563196,-1001307100573,-1001187254179,-1001368097755,-1001222891254,-1001164423875,-1001487436278,-1001428838285,-1001664461759,-1001664461759,-1001415742406,-1001725784523,-1001718523021,-1001459269318,-1001796946839]
@run_async
@restricted
...
def poll(update, context):
    ...
    if reaaa.match("https://t.me/.*?/\d{1,}((:|\n){1,}https://t.me/.*?/\d{1,}){1,}",update.message.text):
    	...
    	for x in range(len(text[2:])//2):
    	    try:
	                   mes=check_mess(get_mess_py("@"+text[2*x+1],int(text[2*x+2])).text,[])
	                   keyboard1=([InlineKeyboardButton(str("⚙️"),callback_data="Link"+str(text[2*x+2])+"_"+str(len(mes))+"_"+str(0)+"_"+str(text[2*x+1]))])
	                   keyboard2=[]
	                   for z in range(len(mes)//5):
	                       keyboard2=[]
	                       for yy in range(5):
	                           keyboard2.append(InlineKeyboardButton(str(num),callback_data="Link"+str(text[2*x+2])+"_"+str(len(mes))+"_"+str(num)+"_"+str(text[2*x+1])))
	                           num+=1
