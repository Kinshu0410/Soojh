...
async def job2(x,client:Client,message:Message):
	...
	try:
		...
		time.sleep(5)
		await app.send_message(int(x),col.find_one({"data":{"$type":"array"}})["data"][Nu[0]][list(col.find_one({"data":{"$type":"array"}})["data"][Nu[0]].keys())[0]],disable_web_page_preview=True)
		time.sleep(5)
