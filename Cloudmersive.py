...
app=Client("my_account",session_string="BQDQx-MAhhRiSyO62bIUC2Sx-1jAg8DtsztvGBU7GcETzmeQMhn3KHvdRPJWlbzf_HETpgQhUrNLehvAeyEK06zutylpEA_uAfD3Nc3xid-e5EetFNjUvOZsQRXJaesgKih4mz2sQ4TrI9y5z02TQzihA6hr4l3JBhfJKYhwaWXFPbCYat3oQNVlBTA54TVtgjYHtDGAvOoSj9AtkE-5txwcbS3XkFAOYuQzaQTbcofKo2pZsmhBR_96mr9YlNkk6W4JNLDqqqSMVUdyA5PRvQiY2IgQ-a3ycQt14spZsBx7PEhDK8XZsnfPHqRR8P-Y_cNya7tJI_PewY36Pf6GGoPbiHAKWQAAAAAqZYQtAA",api_id="13682659",api_hash="b984d240c5258407ea911f042c9d75f6")
...
async def job2_partener1(client:Client,message:Message):
        ...
        for x in range(int(xx[0]),int(xx[1])+1):
            ...
            try:
            	...
            	print(mess1)
            	off_set=None
            	for opi in (mess1.options):
            		for xxxx in range(mess1.total_voter_count//50+1):
            		    mess2=await app.invoke(functions.messages.GetPollVotes(peer=await app.resolve_peer(message.chat.id),id=x,limit=mess1.options[0].voter_count,option=opi.data,offset=off_set))
