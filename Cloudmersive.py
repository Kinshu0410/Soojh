...
app=Client("my_account",session_string="BQDQx-MAGHAA0GZZ9PfYPQGmMSgoyv9tmpkEIy-jNE0Pr74-d0_ancrw7xb12vWO6aP81bI1s-kSUL12ENqujFeFE5tYT-y5C_Yxdja3qxEPFVRmq6zUHYVy213f4F-9VYAALWIQDCK0ttI9Em7TXaZ-E2IT5hgR07nfjOlHGxTb299hUGrY2cIECi5ZYz8cp_Y1F8ysQh80bbMhwT1-X10vjIHsukusHY5bm9S9L8QWttxTHj2FYxNVA7Y92w_ex5_K2Zs2dDiKEwNUI9Msij93G2zZyTsE8iie-ffe0FEbfJnxrlF42jVLqPjp9oPJbLFDV7a3xVpviVlaYS_LQm3iY-UU6gAAAAAqZYQtAA",api_id="13682659",api_hash="b984d240c5258407ea911f042c9d75f6")
...
async def job2_partener2(client:Client,message:Message):
        ...
        #Drive_OCR(body).update(id)
        try:
            replies=Drive_OCR({ "requests": [ { "updateDocumentStyle": { "documentStyle": { "background":{"color":{"color": {"rgbColor": {"red": 1,"green": 1,"blue": 1}}}},"marginTop": { "magnitude": 10, "unit": "PT" },"marginBottom": { "magnitude": 10, "unit": "PT" },"marginLeft": { "magnitude": 10, "unit": "PT" }, "marginRight": { "magnitude": 10, "unit": "PT" },"marginHeader": { "magnitude":10, "unit": "PT" },"marginFooter": { "magnitude": 10, "unit": "PT" },"pageSize": {"height":{"magnitude":(int(zzzz)-int(zzzz)//10+2)*10, "unit": "PT"},"width":{"magnitude": 210, "unit": "PT"}}}, "fields": "*" } }] } ).update(id)
            print(Drive_OCR( { "requests": [ { "insertPageBreak": {"location": {"segmentId": "", "index":count}} } ] }).update(id))
            replies=Drive_OCR({  "requests": [{"createFooter": {"type": "DEFAULT"}}, ] }).update(id)
