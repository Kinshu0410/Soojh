...
async def stop_quiz(client:Client,message:Message):
	ciid=await app.get_chat_members(message.chat.id, filter="administrators")
	print(ciid)
	cid=[]
	for member in await app.get_chat_members(message.chat.id, filter="administrators"):
		cid.append(member.user.id)
	if str(message.chat.id)  in  cid & str(message.chat.id)  in clientmongo["group_schedule"].list_collection_names() :
		print("स्टॉप")
