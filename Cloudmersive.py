...
async def stop_quiz(client:Client,message:Message):
	if str(message.chat.id)  in await app.get_chat_members(message.chat.id, filter="administrators") & str(message.chat.id) in clientmongo["group_schedule"].list_collection_names():
		print("स्टॉप")
